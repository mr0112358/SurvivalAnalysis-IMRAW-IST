...
## KM
S_KM = summary(survfit(Surv(time=X, event = delta, type = "right")~1),
               type = "kaplan-meier",
...
## Add tags
tags = cbind(c("S(8)", "S(12)", "S(16)", "S(8)", "S(12)", "S(16)"),
             c("MLE","MLE","MLE","KM","KM","KM"))
colnames(tags) = c("SurvFunc", "Method")
S_est = data.table(cbind(tags, S_est))
S_est_long = data.frame(melt(S_est, measure.vars = 3:502, variable.name = "COLS", value.name = "Estimate")[,COLS:=NULL])
S_est_long$Estimate = as.numeric(S_est_long$Estimate)

ggplot(S_est_long, aes(x=Estimate, fill = Method))+
  geom_histogram(color = "black", alpha =0.5)+
  facet_grid(Method~SurvFunc)
